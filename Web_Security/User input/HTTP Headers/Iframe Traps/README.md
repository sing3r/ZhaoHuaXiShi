...
- **影响维度**：[x] 信息泄露 [x] 身份伪造 [x] 完整性破坏
- **风险等级**：**高** —— 将一次性 XSS 升级为持续性会话劫持
...
**技术机制**：XSS 注入全屏 iframe 加载当前页面，在 iframe 内注册全局事件监听器，劫持所有用户交互。
...
### 2. Modernised Trap（2024+ 现代化变体）

**新增能力**：

#### 2.1 Navigation API 集成
...
### 3. Overlay & Skimmer Usage（支付页面覆盖与数据窃取）
...
```javascript
// 1. 鼠标离开视口时重新聚焦 iframe → 阻止用户点击浏览器 UI
...
### 链 1：Stored XSS → Iframe Trap → 完整账户接管
...
### 链 2：Reflected XSS → Iframe Trap → Session 持久化
...
### 链 3：Iframe Trap + Clickjacking → 多层劫持
...
```
参考：`Clickjacking/README.md`

### 链 4：Iframe Trap + Payment Skimmer → 金融数据窃取
...
### 链 5：Iframe Trap + XS-Search → 跨源信息窃取
...
- [Clickjacking](../Clickjacking/) — 与 iframe trap 的组合攻击（透明覆盖 + 导航劫持）
- [XSS (Cross-Site Scripting)](../../Reflected%20Values/XSS/) — Iframe Trap 的前提攻击
- [XS-Search](../../Reflected%20Values/XS-Search/) — iframe 时间侧信道信息窃取
- [CSRF](../../Forms-WebSockets-PostMsgs/CSRF/) — 同为跨站操作劫持，防御机制互补
