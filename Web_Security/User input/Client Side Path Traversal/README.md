# 0x01 客户端路径遍历 (CSPT) 攻击深度实战手册

> **风险等级：高危**
> 本手册系统化梳理客户端路径遍历 (Client-Side Path Traversal, CSPT) 攻击技术，揭示其作为 **新型 OSRF (On-Site Request Forgery) 载体** 的实战价值。核心在于利用前端路由机制缺陷，操控浏览器向同一源的敏感端点发起 **经认证的请求**。红队人员应将此技术与缓存投毒、CSRF 形成组合拳，实现账户接管 (ATO) 等高价值攻击。
...
### 6.1.1 账户接管实战（Cache Deception + CSPT）
**攻击流程**：
...
### 6.1.2 Grafana CVE-2025-4123 深度分析
**漏洞点**：
插件资产加载路径处理缺陷：
```javascript
// 漏洞代码片段
...
> 本手册内容持续更新，建议结合目标应用的 **前端路由框架特性**（React/Vue/Angular）进行针对性测试。**成功关键在于精确测绘前端如何处理用户输入并构建后续请求**，而非依赖通用 Payload。