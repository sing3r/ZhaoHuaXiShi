...
> **实战对照**：本攻击模型的成立取决于伪造报文能否以**独立响应**形态跨越请求边界到达。实测中（CL.TE + HEAD 响应长度对齐，对齐经双重验证精确），伪造报文作为反射端点响应的尾部嵌入时**从未被投递**（多次实验 0%，确定性失败）——推断机制为：目标代理按请求读取响应，底层读入超过 Content-Length 的超额数据随请求处理缓冲丢弃，嵌入响应尾部的伪造报文前缀被吞、永远残缺；能存活并被投递的孤儿（HEAD 响应头、后一请求的真实响应）均为独立到达、落在请求读取窗口之外的数据。完整实战分析（含走私构造、对齐验证、0% 失败实验、推断模型与已证实/不可观测边界的区分）见 [CL.TE 请求走私下的响应分割攻击失败根因分析](./CL.TE%20请求走私下的响应分割攻击失败根因分析.md)。
