...
> **实战对照**：本攻击模型隐含一个关键前提——**代理保留跨请求的孤儿响应**（响应队列投毒的成立条件）。若目标代理为响应感知型（对响应做解析与头部改写，且每个请求-响应周期结束时清理连接缓冲中的超额数据），伪造响应孤儿将在投递给下一个受害者前被丢弃，攻击结构性不可行。完整实战分析（含 CL.TE 走私、HEAD 长度精确对齐的验证、WAF 孤儿清理机制判定）见 [CL.TE 请求走私下的响应分割攻击失败根因分析](./CL.TE%20请求走私下的响应分割攻击失败根因分析.md)。
