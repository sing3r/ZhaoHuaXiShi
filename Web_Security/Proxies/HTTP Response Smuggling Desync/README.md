...
> **实战对照**：实测中（CL.TE + HEAD 响应长度对齐，对齐经双重验证精确），伪造报文作为反射端点响应的尾部嵌入时**从未被投递**（大量实验 0%，CL:3260 与 CL:0 同）——但走私、错位均成功（HEAD 孤儿与后一请求的真实响应可被投递）。**深层机制未解决**：三个候选模型（状态机不复位 / 周期清理 / 读入超额丢弃）均无法无张力解释全部观测，已证实/不可观测边界、自我反驳、未解问题与判别实验设计完整记录于 [CL.TE 请求走私下的响应分割攻击失败根因分析](./CL.TE%20请求走私下的响应分割攻击失败根因分析.md)（§0x03），再遇同类环境可直接按 §3.7 判别实验定位。
