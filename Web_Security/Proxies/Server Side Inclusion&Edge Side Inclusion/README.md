...
## 1.3 反射型 SSI 注入与反射型 XSS 的对比

SSI 注入常被称作 **server-side XSS（服务端 XSS）**——两者攻击链的前半段完全相同：用户输入经反射或存储点进入 HTML 页面内容流、且未被转义。分歧发生在**谁解释这段输入**：

```plaintext
用户输入 ──反射 / 存储──> 页面内容流（未转义）
                              │
          XSS：由受害者浏览器（JS 引擎）解释 —— 响应到达后执行
          SSI：由 Web 服务器输出过滤器解释 —— 响应发送前执行
```

| 维度 | 反射型 XSS | 反射型 SSI |
|------|-----------|-----------|
| 输入路径 | 反射 / 存储点，未转义 | 同左（完全相同） |
| 解释器 | 受害者浏览器 JS 引擎 | Web 服务器（Apache `mod_include` / Nginx `ngx_http_ssi_filter_module`） |
| 执行时机 | 响应到达浏览器后 | 服务器发送响应前（输出过滤器阶段） |
| 触发语法 | `<script>` 等，需处于可执行的 HTML/JS 上下文 | `<!--#directive ... -->`——本身是合法 HTML 注释，易被开发者当作无害文本放过 |
| 上下文要求 | 高——需逃逸至可执行上下文（属性内需先闭合引号） | 无——过滤器按原始字节流扫描，属性值内同样命中 |
| 后果 | 客户端侧：会话劫持、钓鱼 | 服务器侧：RCE、任意文件读取、变量/环境泄露、内部 include |
| 影响面 | 单个受害者浏览器 | 服务器本身 + 所有请求该页面的用户 |
| 转义防御 | 输出 HTML 实体转义有效 | 输出 HTML 实体转义同样有效（过滤器不识别 `&lt;`，字节流中无 `<!--#` 即不触发） |

与 XSS 的两处机制差异值得单独强调：

**差异一——字节层扫描，无需突破上下文。** SSI 过滤器扫描的是响应原始字节流而非 HTML 语法树，`<!--#` 序列出现在任何位置都会被解析。只要应用只转义引号而不转义 `<`，输入即使落在 HTML 属性值内也会命中：

```html
<!-- 场景：应用仅转义 "，不转义 < -->
<input value="<!--#exec cmd='id' -->">
<!-- XSS 视角：属性值内 <script> 不构成标签，需先闭合引号才生效 -->
<!-- SSI 视角：字节流中存在 <!--#...--> 即触发，服务器执行 id -->
```

**差异二——服务器能力决定探测方向。** `#exec` 属于 Apache `mod_include` 指令集；Nginx 的 `ngx_http_ssi_filter_module` **不支持 `#exec`**，仅提供 `echo` / `include` / `set` / `if` 等指令（参数值支持 Nginx 变量求值）。因此 Nginx 场景下 SSI 的利用重心转向变量值泄露与内部 location include，而非命令执行——§1.2 的 HackerOne 370094 案例正是这条线上的边界问题。

**探测对照与难点**：

```html
<!--#echo var="DATE_LOCAL" -->   响应中出现服务器本地时间 → SSI 注入确认
<script>alert(1)</script>       浏览器弹窗 → XSS 确认
```

反射型 SSI 尚有一层前置：响应必须经过 SSI 过滤器（`.shtml` / `.shtm` / `.stm` 扩展名、Nginx `ssi on`、Apache `SetOutputFilter INCLUDES`）。同样的反射点在未启用 SSI 的目标上只会表现为 XSS——两者可共存也可各自独立出现，需分别验证。另一个探测难点：SSI 指令本身是 HTML 注释，**失败时浏览器端无任何视觉反馈**（不像 XSS 会弹窗），需靠响应 diff 或外带请求确认。
