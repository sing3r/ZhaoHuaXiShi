...
Flask 默认使用 **itsdangerous** 库对客户端 session cookie 进行 URL 安全的签名序列化。关键区分：Flask cookie 是**签名**（signed）而非**加密**（encrypted）——任何人都可以解码查看内容，但没有密钥无法修改。

Cookie 由三部分组成，以 `.` 分隔：`payload.timestamp.signature`。payload 部分为 Base64 编码的序列化数据。
...
**自动从服务器获取 Cookie（`--server` 参数）：**

```bash
flask-unsign --decode --server 'https://www.example.com/login'
# [+] Successfully obtained session cookie: eyJsb2dnZWRfaW4iOmZhbHNlfQ.XDuWxQ.E2Pyb6x3w...
