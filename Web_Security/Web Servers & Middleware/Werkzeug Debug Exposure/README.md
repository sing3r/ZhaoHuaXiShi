...
PIN 并非完全由算法决定 — 源代码首先检查 **`WERKZEUG_DEBUG_PIN`** 环境变量。如果设置了该变量：若值为 `"off"` 则完全禁用 PIN；若值为纯数字（可能包含连字符），则直接作为 PIN 使用。**仅当该环境变量未设置时**，才进入以下确定性算法。
...
如 [Werkzeug issue #2833](https://github.com/pallets/werkzeug/issues/2833) 中观察到的，Werkzeug 在遇到包含 **Unicode 字符**（代码点 > 255）的请求头时不会正确关闭连接。

**具体代码路径：**

1. `cpython/Lib/http/server.py` 中的 `send_header()` 方法生成响应头行：
   ```python
   ("%s: %s\r\n" % (keyword, value)).encode('latin-1', 'strict')
   ```
   `latin-1` 编码只能处理 0–255 范围的字符。遇到超出此范围的 Unicode 字符时，`.encode('latin-1', 'strict')` 抛出 `UnicodeEncodeError`。

2. Werkzeug 的 `execute()` 函数在 `werkzeug/serving.py` 中捕获此异常：
   ```python
   try:
   ...
   ```
   **关键问题**：try-catch 块只记录错误，**不做任何其他操作** — 不调用 `self.end_headers()`、不发送响应、不关闭连接。

3. 因此，如果 HTTP 连接创建时带有头部 **`Connection: keep-alive`**，请求体不会被读取且 TCP 连接仍保持打开，请求的 **body** 将被视为**下一个 HTTP 请求** — 实现 CL.0 Request Smuggling。

**触发方式**：向 Werkzeug 发送包含 Unicode 字符（如 `\xFFFF`，URL 编码为 `%ef%bf%be`）的请求头值，例如通过 `Set-Cookie` 或 `color` 等用户可控的回显头。参见 [mizu.re 完整 writeup](https://mizu.re/post/twisty-python)。
